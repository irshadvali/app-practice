...
import { Button, Label, FormControl, TextInput, IDropdownItem, Heading, Text, Dropdown } from "@uitk/react";
import styled, { css } from "styled-components";
import TableComponent from "./TargetingComponent/TableComponent";
...
const programTypeDropDown: IDropdownItem[] = [
  { id: "1", label: "Asthama", value: "asthama" },
  { id: "2", label: "Diabetes", value: "diabetes" },
  { id: "3", label: "NA", value: "na" },
];

const fileTypeDropDown: IDropdownItem[] = [
  { id: "1", label: "Targeting", value: "targeting" },
  { id: "2", label: "Demography", value: "demography" },
];

const statusDropDown: IDropdownItem[] = [
  { id: "1", label: "Executed", value: "executed" },
  { id: "2", label: "Pending Exec.", value: "pending" },
  { id: "3", label: "Analysis Comp", value: "analysiscomp" },
  { id: "4", label: "Error", value: "error" },
  { id: "5", label: "Upload Fail", value: "uploadfail" },
];
...
export const TargetingPage: FC = () => {
  ...
  const [populationItems, setPopulationItems] = useState<any[]>([]);
  const [statusItems, setStatusItems] = useState<any>({});
  const [listIdItem, setListIdItem] = useState<any>({});
  ...
  const sortingTable = (col: keyof FieldType) => {
    const sortedData = [...data].sort((a, b) => {
      if (col === "procdatetime") {
        const dateA = new Date(a[col] as string).getTime();
        const dateB = new Date(b[col] as string).getTime();
        return dateA > dateB ? 1 : -1;
      } else {
        return (a[col] as string).toLowerCase() > (b[col] as string).toLowerCase() ? 1 : -1;
      }
    });
    ...
    return (
      <div className={`panel ${showPanel ? "active" : ""}`}>
        <ReportPanel
          ...
          id={id}
          expandFullPanel={expandFullPanel}
    ...
    <>
      {data ? (
        <div className="mainDiv" id="targeting-div">
              ...
              <div className="row">
                <div className="col-s-3 col-m-3 col-1-3">
                  <FormControl id="text-input">
                    ...
                    <TextInput name="healthplan" value={value.healthplan} onChange={onChange} />
                  </FormControl>
                </div>
                <div className="col-s-3 col-m-3 col-1-3">
                ...
                <div className="col-s-3 col-m-3 col-1-3">
                  <Button className="mr-m mt-base" onPress={onFilter} buttonRef={buttonRef}>Filters</Button>
                  <Button variant="tertiary" className="mr-m mt-base" onPress={reset} buttonRef={buttonRef}>Clear</Button>
                </div>
              </div>
            </Fieldset>
          </FilterPanel>
          <TableComponent
            data={data}
            onPressReport={onClickReport}
            onSortData={sortingTable}
            order={order}
